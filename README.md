...
```ts
measureRender(ui: React.ReactElement & { type: { name?: string } }, options?: 1): Promise<MeasureRenderStats>;
```
...
```ts
writeTestStats(stats: MeasureRenderStats, name: string, outputFilePath: string = config.outputFile): Promise<void>
```
