...
Simply run:
```bash
yarn reassure init --logLevel <verbose|default|silent>
```
...
#### `--logLevel` (optional)
This option allows you to control the level of logs. By default, you will only see errors and warnings if those arise, however, if you would like to have a bit more insight into the process you can run it with `--logLevel verbose`. Alternatively, you might want to skip the logs in their entirety. In that case, you can run the command with `--logLevel silent`. 
...
The next two sections will explain in detail the necessity and structure of the files generated by the cli script.
